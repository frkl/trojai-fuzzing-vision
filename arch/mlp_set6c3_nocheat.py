...
class new(nn.Module):
    def __init__(self,params):
        ...
        self.encoder1=MLP(1143,nh,nh2,nlayers); #+27*22
    ...
    def forward(self,data_batch):
        h=[];
        with torch.no_grad():
            fvs=[fv[:27].contiguous().to(self.w.device).view(-1,1143)[:,:] for fv in data_batch['fvs']]; #+27*22
        
        fvs=[self.encoder1(fv).mean(dim=0) for fv in fvs]
