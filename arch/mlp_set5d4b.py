...
class new(nn.Module):
    def __init__(self,params):
        ...
        self.encoder1b=MLP(27*3*2,nh,nh2,nlayers)
        self.encoder2=MLP(8*nh2+nh2,nh3,2,nlayers3);
    ...
    def forward(self,data_batch):
        with torch.no_grad():
            ...
            for i in range(len(data_batch['fvs_fg_clean'])):
                ...
                fvb=torch.cat([data_batch[s][i][:27].to(self.w.device) for s in ['fvs_indu_clean','fvs_indu_triggered']],dim=-1)
                nt,nc,nh=fvb.shape
                fvb=fvb.permute(1,0,2).contiguous().view(nc,nt*nh)
                fvsb_.append(fvb)
        ...
        fvsb=[self.encoder1b(fvb).mean(dim=0) for fvb in fvsb_];
        hb_=torch.stack(fvsb,dim=0);
        
        h=torch.cat([vector_log(h_),vector_log(hb_)],dim=-1)
