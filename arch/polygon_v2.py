    ...
    def forward(self,I,aug=True):
        ...
        if aug:
            ...
            for _ in range(N):
                ...
                #smol rotation
                theta=self.theta+torch.Tensor(1).uniform_(-3.14/30,3.14/30).to(alpha.device)
                #5% imsz offset
                pad=0.1;
                offset=torch.Tensor(2).uniform_(-pad,pad).to(alpha.device)+self.offset;
                t=torch.stack([sz*torch.cos(theta),-sz*torch.sin(theta),offset[0:1]/sz,sz*torch.sin(theta),sz*torch.cos(theta),offset[1:2]/sz],dim=0).view(2,3);
                transform.append(t);
