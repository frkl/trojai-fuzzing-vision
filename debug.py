...
def loss_and_grad(interface,example):
    ...
    for name,param in interface.model.named_parameters():
        if param.grad is None:
            g.append({'name':name,'grad':param.data.cpu()*0})
        else:
            g.append({'name':name,'grad':param.grad.data.cpu()})
    
    return {'loss':float(loss),'grad':g}

data_clean=[loss_and_grad(interface,ex_clean[i]) for i in range(len(ex_clean))];
data_clean2=[loss_and_grad(interface,ex_clean2[i]) for i in range(len(ex_clean2))];
data_poisoned=[loss_and_grad(interface,ex_poisoned[i]) for i in range(len(ex_poisoned))];
data_poisoned2=[loss_and_grad(interface,ex_poisoned2[i]) for i in range(len(ex_poisoned2))];
data_poisoned3=[loss_and_grad(interface,ex_poisoned3[i]) for i in range(len(ex_poisoned3))];
...
def correlate(data1,data2):
    corr=[]
    for i,x in enumerate(data1['grad']):
        name=data1['grad'][i]['name']
        g0=data1['grad'][i]['grad'].view(-1)
        g1=data2['grad'][i]['grad'].view(-1)
        r=(F.normalize(g0,dim=-1)*F.normalize(g1,dim=-1)).sum()
        corr.append({'name':name,'r':r})
...
fnames=[x['fname'] for x in ex_clean+ex_poisoned]
write_csv('debug.csv',torch.cat((corr_clean,corr_poisoned),dim=-1),fnames,layer_names)
